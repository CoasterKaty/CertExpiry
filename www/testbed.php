...
$thingBtn = new navigationItem('Next', 'sub', 'right');
$thingBtn->link = 'testbed2.php';
$thingBtn->selected = 1;
$thisPage->addNavigation($thingBtn);
 ...
 $urlTable = new pageTable();
	...
	for ($i = 1; $i < 20; $i++) {
		...
		$tableRow->column['Thing2']->text = 'A row in a table ' . $i . ' a long row in a long row in a long row in a long row in a long row in a long row in a long row in a long row in a long row in a long row in a long row in a long row in a table';
